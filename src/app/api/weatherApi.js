...
export async function getWeatherApi(cityValue, cord) {
	let API_URL = null;
	if (cityValue)
		API_URL = `https://api.openweathermap.org/data/2.5/forecast?q=${cityValue}&units=metric&appid=${WEATHER_KEY}&lang=es`;
	else
		API_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${cord.latitude}&lon=${cord.longitude}&units=metric&appid=${WEATHER_KEY}&lang=es`;
	if (API_URL)
		return axios
			.get(API_URL)
			.then(response => {
